		...
		persist(
			(set) => ({
				...
				removePokemon(nickname) {
					set((state) => ({
						pokemons: state.pokemons.filter((pokemon) => pokemon.nickname !== nickname),
					}));
					toast.success('Pokemon was released');
				},
				updatePokemon(newNickname, oldNickname) {
					set((state) => ({
						pokemons: state.pokemons.map((pokemon) =>
							pokemon.nickname === oldNickname ? { ...pokemon, nickname: newNickname } : pokemon
						),
					}));
				},
			}),
